  ...
  update() {
    if (enableGridLines) {
      ...
      for (int i = 0; i < gridLineAmount; i++) {
        // Label grid lines
        gridLineValue = _max - (((_max) / (gridLineAmount - 1)) * i);

        String gridLineText = gridLinelabel != null
            ? gridLinelabel!(gridLineValue)
            : (gridLineValue/1000).toStringAsFixed(0);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (enableGridLines) {
      ...
      for (int i = 0; i < gridLineAmount; i++) {
        ...
        canvas.drawLine(
            Offset(0.0, gridLineY), Offset(width*1.06  , gridLineY), gridPaint);

        // Label grid lines
        gridLineTextPainters[i]
            .paint(canvas, Offset(0.0, gridLineY - 6.0));
      }
    }
    ...
    if (useCubicSmoothing) {
      ...
      for (int i = 1; i < normalized.length; i++) {
        path.lineTo(normalized[i].dx -10, normalized[i].dy);
      }
    }
    ...
    if (fillMode != FillMode.none) {
      ...
      if (fillMode == FillMode.below) {
        ...
        fillPath.lineTo(31.0, size.height);
    ...
    if (averageLine) {
      ...
      if (averageLabel) {
        var averageVal = dataPoints.reduce((a, b) => a + b);
